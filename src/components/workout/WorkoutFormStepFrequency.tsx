'use client';

import { FREQUENCIES } from '@/lib/constants/frequencies';
import { WorkoutFormData } from './WorkoutForm';

interface Props {
  data: WorkoutFormData;
  onChange: (updates: Partial<WorkoutFormData>) => void;
}

export default function StepFrequency({ data, onChange }: Props) {
  return (
    <div>
      <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">
        How often do you plan to do this?
      </h2>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
        Your intended frequency — we&apos;ll track whether reality matches.
      </p>
      <div className="space-y-2">
        {FREQUENCIES.map((freq) => {
          const isSelected = data.intended_frequency === freq.value;
          return (
            <button
              key={freq.value}
              type="button"
              onClick={() => onChange({ intended_frequency: freq.value })}
              className={`w-full p-4 rounded-xl border text-left transition-colors touch-manipulation ${
                isSelected
                  ? 'border-blue-400 bg-blue-50 dark:border-blue-600 dark:bg-blue-900/30'
                  : 'border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 hover:border-gray-300 dark:hover:border-gray-600'
              }`}
            >
              <div className="flex items-center justify-between">
                <div>
                  <span className={`text-sm font-semibold ${
                    isSelected ? 'text-blue-700 dark:text-blue-300' : 'text-gray-900 dark:text-gray-100'
                  }`}>
                    {freq.label}
                  </span>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                    {freq.description}
                  </p>
                </div>
                {isSelected && (
                  <div className="w-5 h-5 rounded-full bg-blue-600 dark:bg-blue-500 flex items-center justify-center">
                    <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                    </svg>
                  </div>
                )}
              </div>
            </button>
          );
        })}
      </div>
    </div>
  );
}
